...
def compile_cmd(src, obj, debugging=False):
    return 'g++ %s -Wall -Wextra -Wpedantic -I"include" -c %s -o %s' \
        %(debug_flag(debugging), src, obj)

def link_cmd(executable, objs, debugging=False):
    return 'g++ %s -o %s %s -lm' %(debug_flag(debugging), executable, ' '.join(objs))
...
def valgrind_cmd(program_args):
    return f'valgrind --leak-check=full --show-leak-kinds=all \
        ./build/debug_posix/blerg {program_args}'
...
def valgrind():
    if os.name == 'nt':
        ...
        if build(True):
            program_args = ' '.join(sys.argv[2:])
            if program_args == '':
                program_args = '--test'
            subprocess.run(valgrind_cmd(program_args), shell=True)
